...
export class LocalSSHMetricsReporter {
    ...
    reportConfigStatus(status: 'success' | 'failure', failureCode?: string) {
        ...
        return addCounter(this.metricsHost, 'vscode_desktop_local_ssh_config_count', { status, failure_code: failureCode ?? 'Unknown' }, 1, this.logService);
    }

    reportPingExtensionStatus(status: 'succeed' | 'failure') {
        return addCounter(this.metricsHost, 'vscode_desktop_ping_extension_server_count', { status }, 1, this.logService);
    }
    ...
    reportConnectionStatus(phase: 'connected' | 'connecting' | 'failed', failureCode?: string) {
        ...
        return addCounter(this.metricsHost, 'vscode_desktop_local_ssh_count', { phase, failure_code: failureCode ?? 'Unknown' }, 1, this.logService);
    }
}
